...
def eval_historical_average(traffic_reading_df, period):
    ...
    logger.info('\t'.join(['Model', 'Horizon', 'RMSE', 'MAPE', 'MAE']))
    line = 'HA\t%d\t%.2f\t%.2f\t%.2f' % (0, rmse, mape * 100, mae)
    logger.info(line)
    logger.info('Make prediction using the same day of the week data for the last four weeks')
...
def main_refer_eval(args):
    traffic_reading_filename="data/"+args+".h5"
    traffic_reading_df = pd.read_hdf(traffic_reading_filename)
    eval_static(traffic_reading_df)
    eval_historical_average(traffic_reading_df, period=7 * 24 * 12)
    eval_var(traffic_reading_df, n_lags=3)
...
if __name__ == '__main__':
    #원래 아래 코드 실행되는건데 start로 실행하려고 맨 위로 옮김
    #logger = utils.get_logger('data/model', 'Baseline')
    parser = argparse.ArgumentParser()
    parser.add_argument('--traffic_reading_filename', default="data/Seoul_all.h5", type=str,
